...
$(function(){
	if(boo){
		$.ajax({
			url:"http://localhost:8089/jscucc/findByIdAdmin.do",
			type:"post",
			data:{"id":admin_id},
			datatype:"json",
	...
	$(document).on('click','#role_list',function(){
		...
		$("#d").html("");
		$.ajax({
			url:"http://localhost:8089/jscucc/findAdminRole.do",
			type:"post",
			data:{"":""},
			dataType:"json",
			success:function(result){
	...
	$("#adminInfo").click(function(){
		...
		$("#infoh2").html("个人信息");
		$.ajax({
			url:"http://localhost:8089/jscucc/findEmpInfo.do",
			type:"post",
...
//							"<p id='infoError'></p>";
	...
	$(document).on('click','#bcxg',function(){
		...
		if(boo){
			$.ajax({
				url:"http://localhost:8089/jscucc/updateEmpInfo.do",
				type:"post",
	...
	$("#updatePwd").click(function(){
		...
		$("#cmt").click(function(){
			...
			if(boo){
				$.ajax({
					url:"http://localhost:8089/jscucc/updatepwd.do",
					type:"post",
	...
	$(document).on('click',"#add_admin1",function(){
		...
		aCancle[1].onclick=function(){
			...
			if(boo){
				$.ajax({
					url:"http://localhost:8089/jscucc/addAdmin.do",
					type:"post",
...
//修改管理员操作
 function updateadmin(btn){
	 ...
	 $("#adminPhone").val(phone);
		...
		aCancle[1].onclick=function(){
			...
			if(boo){
				$.ajax({
					url:"http://localhost:8089/jscucc/updateAdmin.do",
					type:"post",
 ...
 function deladmin(btn){
	 ...
	 if(account==admin.account){
		 ...
		 $.ajax({
			 url:"http://localhost:8089/jscucc/delAdmin.do",
			 type:"post",
